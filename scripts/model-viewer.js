document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!WebGLRenderingContext) {
        container.innerHTML = '<div class="error-message">Ваш браузер не поддерживает WebGL. Пожалуйста, обновите браузер или используйте другой.</div>';
        return;
    }
    ...
    loader.load(
        ...
        (error) => {
            ...
            container.removeChild(loadingIndicator);
            container.innerHTML = '<div class="error-message">Ошибка загрузки 3D модели. Проверьте путь к файлу и консоль для деталей.</div>';
        }
    );
